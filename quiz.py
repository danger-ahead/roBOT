...
class Quiz:
        ...
        while position < len(lines):
            ...
            if lines[position].strip() == "":  # blank line
                ...
                if question is not None and answer is not None:
                    q = Question(
                        question=question, answer=answer, category=category, regex=regex
                    )
            ...
            if lines[position].strip().lower().startswith("category"):
                category = (
                    lines[position].strip()[lines[position].find(":") + 1 :].strip()
                )
    ...
    async def start(self, channel):
        ...
        if self.__running:
            # don't start again
            await channel.send(
                "Quiz already started in channel {}, you can stop it with _qstop".format(
                    self._channel.name
                )
            )
        else:
    ...
    async def ask_question(self, message, channel):
        ...
        if self.__running:
            ...
            await channel.send(
                "Question {}: {}".format(
                    len(self._asked), self.current_question.ask_question()
                )
            )
    ...
    async def answer_question(self, message, channel):
        ...
        if self.__running and self.current_question is not None:
            ...
            if self.current_question.answer_correct(message.content):
                ...
                await message.channel.send(
                    "Well done, {}, the correct answer was: {}".format(
                        message.author.name, self.current_question.get_answer()
                    )
                )
                self.current_question = None
